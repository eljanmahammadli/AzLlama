...
from kenlm_tokenization import preprocess_text
...
class ModifyingDocuments:
    ...
    @staticmethod
    def tokenization(document, join_on_whitespace):
    ...
    def get_words_from_document(document, lower_case, strip_characters):
        ...
        words = ModifyingDocuments.tokenization(
            document_normalized, join_on_whitespace=False
        )
...
class Filtering:
    ...
    def compute_word_repetition_ratio(
        document, strip_characters, word_repetition_length
    ):
        def get_freq_word_ngrams(document, strip_characters, n):
            words = ModifyingDocuments.get_words_from_document(
                document,
                # sentencepiece_model_tok,
                lower_case=True,
    ...
    #         replace_unicode_punctuation=True,
    #     )
    #     document = ModifyingDocuments.tokenization(document, join_on_whitespace=True)
    ...
    @staticmethod
    def compute_perplexity_score(document, kenlm_model):
        document = preprocess_text(document)
        sentences = sent_tokenize(document)

        total_log_prob, total_words = 0.0, 0
        for sentence in sentences:
            words = word_tokenize(sentence)
            total_words += len(words) + 1  # +1 for EOS
            log_prob = kenlm_model.score(" ".join(words))
    ...
    def check_perplexity(
        ...
        cond = True
        if kenlm_model:
            score = Filtering.compute_perplexity_score(document, kenlm_model)
